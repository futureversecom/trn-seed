...
/// Helper function to create bounded vec of ERC721Ownership
pub fn create_owned_tokens(
	owned_tokens: Vec<(AccountId, Vec<SerialNumber>)>,
) -> BoundedVec<ERC721Ownership<Test>, MaxTokensPerCollection> {
	let mut token_ownership: BoundedVec<ERC721Ownership<Test>, MaxTokensPerCollection> =
		BoundedVec::default();
	for (owner, serial_numbers) in owned_tokens {
		...
		let new_token_ownership = ERC721Ownership::new(owner, serial_numbers_bounded);
...
fn add_user_tokens_works() {
	TestExt::default().build().execute_with(|| {
		...
		let expected_owned_tokens: BoundedVec<ERC721Ownership<Test>, MaxTokensPerCollection> =
			BoundedVec::default();
