...
mod get_runtime_call_from_xumm_extrinsic {
	...
	fn test_xumm_get_runtime_call_system_remark() {
		TestExt::<Test>::default().build().execute_with(|| {
			...
			let decoded_call =
				XrplTransaction::get_runtime_call_from_xumm_extrinsic(&scale_encoded_extrinsic)
					.unwrap();
...
mod self_contained_call {
	...
	#[test]
	fn submit_encoded_xumm_transaction_validations() {
		TestExt::<Test>::default().build().execute_with(|| {
	...
	fn extrinsic_cannot_perform_privileged_operations() {
		TestExt::<Test>::default().build().execute_with(|| {
			...
			// executing xumm encoded transaction fails since caller is not root/sudo account
			assert_noop!(
				XrplTransaction::submit_encoded_xumm_transaction(frame_system::RawOrigin::None.into(), BoundedVec::truncate_from(tx_bytes), BoundedVec::default()),
				BadOrigin,
			);
	...
	fn signed_extension_validations() {
		TestExt::<Test>::default()
			...
			.execute_with(|| {
				...
				let xt: mock::UncheckedExtrinsicT = fp_self_contained::UncheckedExtrinsic::new_unsigned(mock::RuntimeCall::XrplTransaction(crate::Call::submit_encoded_xumm_transaction {
				...
				// validate self contained extrinsic fails, nested submit_encoded_xumm_transaction call fails
				let xt: mock::UncheckedExtrinsicT = fp_self_contained::UncheckedExtrinsic::new_unsigned(mock::RuntimeCall::XrplTransaction(crate::Call::submit_encoded_xumm_transaction {
				...
				System::assert_last_event(mock::RuntimeEvent::System(
					frame_system::Event::ExtrinsicSuccess {
						dispatch_info: DispatchInfo {
							weight: Weight::from_ref_time(535_622_000),
