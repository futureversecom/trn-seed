...
mod tests {
	...
	fn serialize_eth_tx_receipt_request() {
		let result = serde_json::to_string(&GetTxReceiptRequest::new(
			EthHash::from_str("0x185e85beb3296c7339954811cc682e3f992573ad3eecd37409e0ed763448d303")
				.unwrap(),
			1,
		))
		.unwrap();
	...
	fn deserialize_eth_block_request() {
		...
		let _result: EthResponse<EthBlock> =
	...
	fn deserialize_eth_transaction_receipt() {
		...
		let _result: EthResponse<TransactionReceipt> =
			serde_json::from_str(response).expect("it deserializes");
	}
	...
	fn deserialize_null_response_as_none() {
		assert_eq!(
			serde_json::from_str::<EthResponse<EthBlockNumber>>(
				r#"{"jsonrpc":"2.0","id":1,"result":null}"#
			)
			.unwrap(),
			EthResponse { id: 1, jsonrpc: "2.0", result: None }
		);
	}
	...
	fn serialize_get_block_by_number_request() {
		let expected =
			r#"{"jsonrpc":"2.0","method":"eth_getBlockByNumber","params":["0x37",false],"id":1}"#;
	...
	fn serialize_get_block_by_number_request_max() {
		let expected = r#"{"jsonrpc":"2.0","method":"eth_getBlockByNumber","params":["0xfffffffffffffffe",false],"id":1}"#;
