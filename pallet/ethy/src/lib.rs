...
pub mod pallet {
	...
	pub type EventBlockConfirmations<T> = StorageValue<_, u64, ValueQuery>;
	...
	pub type DelayedEventProofsPerBlock<T> = StorageValue<_, u64, ValueQuery>;
	...
	pub type PendingEventProofs<T> = StorageMap<_, Twox64Concat, EventProofId, EthySigningRequest, OptionQuery>;
	...
	pub type PendingClaimStatus<T> = StorageMap<_, Twox64Concat, EventProofId, EventClaimStatus, OptionQuery>;
	...
	pub type EthCallRequests<T> = StorageValue<_, Vec<EthCallId>, OptionQuery>;
	...
	pub type EthCallNotarizationsAggregated<T> = StorageMap<_, Twox64Concat, EthCallId, BTreeMap<CheckedEthCallResult, u32>, OptionQuery>;

	/// EthCallOracle request info
	#[pallet::storage]
	#[pallet::getter(fn eth_call_request_info)]
	pub type EthCallRequestInfo<T> = StorageMap<_, Twox64Concat, EthCallId, CheckedEthCallRequest, OptionQuery>;
