...
use crate::{
	...
	NextNotaryKeys, NotaryKeys, NotaryXrplKeys, XrplDoorSigners, ETHY_ENGINE_ID,
...
fn process_valid_challenged_event() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				EthBridge::on_initialize(process_at),
				DbWeight::get().reads(2) + 1000.into()
			);
...
fn process_valid_challenged_event_delayed() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(EthBridge::on_initialize(process_at), DbWeight::get().reads(2));
...
fn delayed_event_proof() {
	ExtBuilder::default().build().execute_with(|| {
		...
		let expected_weight: Weight =
			DbWeight::get().reads(3) + DbWeight::get().writes(2) * max_delayed_events;
		assert_eq!(
			EthBridge::on_initialize(frame_system::Pallet::<TestRuntime>::block_number() + 1),
			expected_weight
		);
...
fn multiple_delayed_event_proof() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(
			EthBridge::on_initialize(frame_system::Pallet::<TestRuntime>::block_number() + 1),
			DbWeight::get().reads(3) + DbWeight::get().writes(2) * max_delayed_events as u64
		);
...
fn set_delayed_event_proofs_per_block() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(
			EthBridge::on_initialize(frame_system::Pallet::<TestRuntime>::block_number() + 1),
			DbWeight::get().reads(3) + DbWeight::get().writes(2) * new_max_delayed_events as u64
		);
...
fn notary_xrpl_keys_unchanged_do_not_request_for_xrpl_proof() {
	ExtBuilder::default().active_session_final().build().execute_with(|| {
		...
		assert_eq!(EthBridge::xrpl_notary_set_proof_id(), 0); // No change to XrplNotarySetProofId since no change to NotaryXrplKeys

		// current session is last in era: finishing
		<Module<TestRuntime> as OneSessionHandler<AccountId>>::on_before_session_ending();
		assert_eq!(EthBridge::notary_keys(), next_keys);
		assert_eq!(EthBridge::notary_set_id(), current_set_id + 1);
		assert_eq!(EthBridge::notary_xrpl_keys(), next_keys);
		assert_eq!(EthBridge::xrpl_notary_set_proof_id(), 0);
