...
use seed_pallet_common::ValidatorAdapter;
...
fn process_valid_challenged_event() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				EthBridge::on_initialize(process_at),
				DbWeight::get().reads(0 as Weight) + 1000 as Weight
			);
...
fn process_valid_challenged_event_delayed() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(EthBridge::on_initialize(process_at), DbWeight::get().reads(0 as Weight));
