...
use seed_pallet_common::{
	log, EthCallFailure, EthCallOracle, EthCallOracleSubscriber, EthereumEventBridge,
	EthereumEventClaimSubscriber, FinalSessionTracker as FinalSessionTrackerT,
};

use crate::{types::*, *};

impl<T: Config> EthereumEventBridge for Module<T> {
	...
	/// Request an event proof is generated by notaries with next available proof Id
	fn request_event_proof(
		...
		let eth_abi_encoded_event_for_proving = ethabi::encode(&[
			// event data
			Token::Address(*destination),
			Token::Bytes(event.to_vec()),
			// proof metadata
			Token::Uint(Self::validator_set().id.into()),
		...
		if Self::bridge_paused() {
			DelayedEventProofs::insert(event_proof_id, eth_abi_encoded_event_for_proving);
			Self::deposit_event(Event::ProofDelayed(event_proof_id));
		} else {
			Self::do_request_event_proof(event_proof_id, eth_abi_encoded_event_for_proving);
		}
...
impl<T: Config> Module<T> {
	...
	pub(crate) fn do_event_notarization_ocw(active_key: &T::EthyId, authority_index: u16) {
		...
		// production.
		// TODO: make this FIFO (claims are not sorted)
		for (event_claim_id, event_claim) in EventClaims::iter().take(CLAIMS_PER_BLOCK) {
	...
	fn offchain_try_notarize_event(event_claim: EventClaim) -> EventClaimResult {
		let EventClaim { tx_hash, event_data, event_signature, source_address } = event_claim;
		...
		if tx_receipt.to != Some(source_address) {
		...
		// we can calculate if the block is expired w some high degree of confidence without making
		// a query. time since the event = block_confirmations * ~16 seconds avg
	...
	pub fn validator_set() -> ValidatorSet<T::EthyId> {
		...
		ValidatorSet::<T::EthyId> {
			proof_threshold: T::NotarizationThreshold::get() * validator_keys.len() as u32,
	...
	pub(crate) fn handle_event_notarization(
			...
			(Percent::from_parts(100_u8 - T::NotarizationThreshold::get().deconstruct()))
		{
			...
			if let Some(event_claim) = EventClaims::take(event_claim_id) {
				PendingTxHashes::remove(event_claim.tx_hash);
				Self::deposit_event(Event::Invalid(event_claim_id));

				T::Subscribers::on_failure(
					event_claim_id,
					&event_claim.source_address,
					&event_claim.event_signature,
					&event_claim.event_data,
				);
				return Ok(())
			} else {
		...
		if Percent::from_rational(yay_count, notary_count) >= T::NotarizationThreshold::get() {
			...
			if let Some(event_claim) = EventClaims::take(event_claim_id) {
				...
				// calculate future expiry timestamp and pruning bucket
				let bucket_index = ((T::UnixTime::now().as_secs().saturated_into::<u64>() + ttl) %
					ttl) / BUCKET_FACTOR_S;
				ProcessedTxBuckets::insert(bucket_index, event_claim.tx_hash, ());
				ProcessedTxHashes::insert(event_claim.tx_hash, ());
				PendingTxHashes::remove(event_claim.tx_hash);
				Self::deposit_event(Event::Verified(event_claim_id));

				T::Subscribers::on_success(
					event_claim_id,
					&event_claim.source_address,
					&event_claim.event_signature,
					&event_claim.event_data,
			...
			let log: DigestItem = DigestItem::Consensus(
				...
				ConsensusLog::PendingAuthoritiesChange((
					ValidatorSet {
						...
						proof_threshold: T::NotarizationThreshold::get() * next_keys.len() as u32,
					},
					event_proof_id,
				))
				.encode(),
