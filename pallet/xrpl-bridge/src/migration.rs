use crate::{Pallet, Config, weights::WeightInfo};
use frame_support::{
...
fn migrate_0_to_1() {
    ...
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(have_storage_value(<Pallet<Test>>::name().as_bytes(), storage_item_name, test_storage_key), false);
        put_storage_value(<Pallet<Test>>::name().as_bytes(), storage_item_name, test_storage_key, 123);
        assert_eq!(have_storage_value(<Pallet<Test>>::name().as_bytes(), storage_item_name, test_storage_key), true);    
        try_migrate::<Test>();
        assert_eq!(have_storage_value(<Pallet<Test>>::name().as_bytes(), storage_item_name, test_storage_key), false);
    });
}