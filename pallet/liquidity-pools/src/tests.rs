...
use mock::{
	new_test_ext, LiquidityPools, RuntimeEvent as MockEvent, RuntimeOrigin as Origin, System, Test,
	TestExt,
};
...
fn non_admin_cannot_close_pool() {
	new_test_ext().execute_with(|| {
		assert_noop!(LiquidityPools::close_pool(Origin::signed(create_account(1)), 0), BadOrigin);
	});
}
...
fn admin_can_close_pool_successfully() {
	new_test_ext().execute_with(|| {
		...
		System::assert_last_event(MockEvent::LiquidityPools(crate::Event::PoolClosed { pool_id }));
...
fn cannot_join_pool_if_token_limit_exceeded() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			LiquidityPools::join_pool(Origin::signed(create_account(1)), pool_id, max_tokens + 1),
...
fn claim_reward_should_work() {
	...
	let balances = vec![(LiquidityPools::get_vault_account(0).unwrap(), user_balance * 100)];
...
fn claim_reward_should_fail_if_no_tokens_staked() {
	...
	let balances = vec![(LiquidityPools::get_vault_account(0).unwrap(), user_balance)];
