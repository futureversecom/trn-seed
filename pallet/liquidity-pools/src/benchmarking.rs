...
fn mint_asset<T: Config>(acc: &T::AccountId) -> AssetId
where
...
benchmarks! {
	...
	set_pool_rollover {
		...
		let vault_account = LiquidityPools::<T>::get_vault_account(id).unwrap();
		...
		LiquidityPools::<T>::on_idle(start_block, remaining_weight);
	...
	claim_reward {
		...
		for i in 0..50+50 as u32 {
			...
			LiquidityPools::<T>::on_idle(i.into(), remaining_weight);
		}
	}: _(RawOrigin::Signed(user.clone()), id)
	verify {
		// User reward debt should have increased
		let user_info = PoolUsers::<T>::get(id, user).unwrap();
		assert_eq!(user_info.reward_debt, 10_000_000u32.into());
	}
	...
	rollover_unsigned {
		...
		LiquidityPools::<T>::on_idle(end_block, remaining_weight);
