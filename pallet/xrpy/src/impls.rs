...
impl<T: Config> EthereumEventBridge for Module<T> {
	/// Request verification of a remote event with the following criteria
	/// - triggered by `tx_hash`
	/// - event generated by `source_address` (contract)
	/// - `event_data` matches exactly
	/// - `event_selector`
	...
	/// Request an event proof to be generated by notaries (async)
...
impl<T: Config> Module<T> {
	...
	pub(crate) fn offchain_try_notarize_event(event_claim: EventClaim) -> EventClaimResult {
		let EventClaim { tx_hash, event_data, event_selector, source_address } = event_claim;
		...
		let topic: EthHash = event_selector;
	...
	pub(crate) fn handle_event_notarization(
			...
			(Percent::from_parts(100_u8 - T::NotarizationThreshold::get().deconstruct()))
		{
			...
			if let Some(event_claim) = PendingEventClaims::take(event_claim_id) {
				...
				T::EventClaimSubscribers::on_failure(
					event_claim_id,
					&event_claim.source_address,
					&event_claim.event_selector,
		...
		if Percent::from_rational(yay_count, notary_count) >= T::NotarizationThreshold::get() {
			...
			if let Some(event_claim) = PendingEventClaims::take(event_claim_id) {
				...
				T::EventClaimSubscribers::on_success(
					event_claim_id,
					&event_claim.source_address,
					&event_claim.event_selector,
					&event_claim.event_data,
