...
use seed_pallet_common::{
	log, xrpl::XrplBridge, EthCallFailure, EthCallOracleSubscriber, EthereumBridge,
	FinalSessionTracker as FinalSessionTrackerT,
};
use seed_pallet_common::xrpl::XrplCallOracle;
use seed_primitives::{
	validator::{EventClaimId, EventProofId},
};

use crate::{xrpl_types::*, *};

impl<T: Config> XrplBridge for Pallet<T> {
	...
	fn send_event(
		...
		if Self::bridge_paused() {
			<PendingEventProofs<T>>::insert(event_proof_id, event_proof_info);
...
impl<T: Config> Pallet<T> {
	...
	pub(crate) fn do_event_notarization_ocw(active_key: &T::ValidatorId, authority_index: u16) {
		...
		for event_claim_id in <PendingClaimChallenges<T>>::get().iter().take(CLAIMS_PER_BLOCK) {
			...
			if <EventNotarizations<T>>::contains_key::<EventClaimId, T::ValidatorId>(
				*event_claim_id,
				active_key.clone(),
			) {
	...
	/// Receives the node's local notary session key and index in the set
	pub(crate) fn do_call_notarization_ocw(active_key: &T::ValidatorId, authority_index: u16) {
		// we limit the total claims per invocation using `CALLS_PER_BLOCK` so we don't stall block
		// production
		for call_id in ChainCallRequests::get().iter().take(CALLS_PER_BLOCK) {
			// skip if we've notarized it previously
	...
	pub(crate) fn offchain_try_xrp_call(request: &CheckedChainCallRequest) -> CheckedChainCallResult {
		...
		if latest_eth_block_timestamp == u64::max_value() {
			return CheckedChainCallResult::InvalidTimestamp
		}
		...
		let latest_eth_block_number = match latest_block.number {
			Some(number) => {
				if number.is_zero() || number.low_u64() == u64::max_value() {
					return CheckedChainCallResult::InvalidEthBlock
				}
				number.low_u64()
			},
			None => return CheckedChainCallResult::InvalidEthBlock,
		};
	...
	pub(crate) fn handle_event_notarization(
		...
		<EventNotarizations<T>>::insert::<EventClaimId, T::ValidatorId, EventClaimResult>(
			event_claim_id,
			notary_id.clone(),
			result,
		);
	...
	pub(crate) fn handle_call_notarization(
		...
		let do_callback_and_clean_up = |result: CheckedChainCallResult| {
			match result {
				CheckedChainCallResult::Ok(return_data, block, timestamp) =>
				...
				CheckedChainCallResult::ReturnDataEmpty =>
					T::EthCallSubscribers::on_eth_call_failed(
						call_id,
						EthCallFailure::ReturnDataEmpty,
					),
				CheckedChainCallResult::ReturnDataExceedsLimit =>
					T::EthCallSubscribers::on_eth_call_failed(
						call_id,
