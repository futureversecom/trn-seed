...
use seed_pallet_common::xrpl::XrplBridge;
use seed_primitives::ethy::PendingAuthorityChange;
use seed_primitives::validators::validator::{EventClaimId, EventProofId};

use crate::{xrpl_types::*, *};

impl<T: Config> XrplBridge for Module<T> {
...
impl<T: Config> Pallet<T> {
	...
	pub(crate) fn do_call_notarization_ocw(active_key: &T::ValidatorId, authority_index: u16) {
		...
		for call_id in EthCallRequests::get().iter().take(CALLS_PER_BLOCK) {
			// skip if we've notarized it previously
			if <EthCallNotarizations<T>>::contains_key::<EthCallId, T::ValidatorId>(
				*call_id,
				active_key.clone(),
			) {
	...
	pub(crate) fn handle_call_notarization(
		...
		<EthCallNotarizations<T>>::insert::<EventClaimId, T::ValidatorId, CheckedEthCallResult>(
			call_id,
			notary_id.clone(),
			result,
		);
