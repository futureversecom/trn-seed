	...
	pub fn note(&mut self, witness: &Witness) -> bool {
		...
		let authority_to_index = || -> Option<usize> {
			...
			if maybe_pos.is_none() {
				...
				// this should not happen (i.e. the witness should be invalidated sooner in the
				// lifecycle)
		...
		self.record
				...
				event_digests
					...
					.and_modify(|signatures| {
						...
						authority_to_index().map(|authority_index| {
							signatures.push((authority_index, witness.signature.clone()))
						});
					})
					.or_insert({
						// case 3
			...
			.or_insert({
				...
				authority_to_index().map(|authority_index| {
					digest_signatures
						.insert(witness.digest, vec![(authority_index, witness.signature.clone())])
				});
				digest_signatures
			});
		...
		match self.has_voted.get_mut(&witness.event_id) {
			None => {
				...
				self.has_voted.insert(witness.event_id, vec![witness.authority_id.clone()]);
			},
			Some(votes) => {
