...
describe("Reward", () => {
  ...
  it("load test for rollover", async () => {
    ...
    const liquidityPoolsVaultForPool1 = process.env.LIQUIDITYPOOLS_VAULT_ADDRESS_POOL_1;
    const liquidityPoolsVaultForPool2 = process.env.LIQUIDITYPOOLS_VAULT_ADDRESS_POOL_2;
    // fund vault account
    await finalizeTx(
      alith,
      api.tx.utility.batch([
        api.tx.balances.transfer(liquidityPoolsVaultForPool1, testUsers.length * joinPoolAmount),
        api.tx.balances.transfer(liquidityPoolsVaultForPool2, testUsers.length * joinPoolAmount),
      ]),
    );
    console.log(`vault account ${liquidityPoolsVaultForPool1} and ${liquidityPoolsVaultForPool2} funded`);
