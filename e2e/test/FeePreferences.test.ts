...
describe("Fee Preferences", function () {
  ...
  describe("Batch EVM Support", function () {
    ...
    it("Pays fees for batch with multiple EVM calls in non-native token", async () => {
      // Use alith as sender like the working test
      const sender = alith.address;
      //const erc20PrecompileAddress = assetIdToERC20ContractAddress(feeTokenAssetId);
      const erc20PrecompileAddress = xrpERC20Precompile.address;
      ...
      // Create batch call containing multiple EVM calls
      const batchCall = api.tx.utility.batch([evmCall1, evmCall2]);
    ...
    it("Pays fees for batch with mixed EVM and non-EVM calls in non-native token", async () => {
      ...
      // Create batch call containing mixed calls
      const batchCall = api.tx.utility.batch([remarkCall1, evmCall, remarkCall2]);
    ...
    it("Pays fees for futurepass proxy_extrinsic with batch containing EVM calls in non-native token", async () => {
      // Use alith like the working test to simplify and make it more likely to work
      // Create futurepass for alith
      await finalizeTx(alith, api.tx.futurepass.create(alith.address));
      const futurepassAddress = (await api.query.futurepass.holders(alith.address)).toString();
      ...
      // Create batch call containing mixed calls
      const batchCall = api.tx.utility.batch([evmCall1, remarkCall1, evmCall2, remarkCall2]);
