...
import {
    ALITH_PRIVATE_KEY,
    BOB_PRIVATE_KEY,
    ERC1155_PRECOMPILE_ABI, FUTUREPASS_PRECOMPILE_ABI,
    SFT_PRECOMPILE_ABI,
...
describe("ERC1155 Precompile", function () {
    ...
    async function createToken(initialIssuance: number, tokenOwner: Wallet = bobSigner) {
        ...
        const maxIssuance = 0;
        const tx = await erc1155Precompile.connect(bobSigner).createToken(tokenName, maxIssuance);
        const receipt = await tx.wait();
        const serialNumber = (receipt?.events as any)[0].args.serialNumber;

        if (initialIssuance > 0) {
            // Mint tokens with initialIssuance to tokenOwner
            const mintTx = await erc1155Precompile.connect(bobSigner).mint(tokenOwner.address, serialNumber, initialIssuance);
            await mintTx.wait();
        }
    ...
    it("mintBatch", async () => {
        ...
        const mintTx = await erc1155Precompile.connect(bobSigner).mintBatch(receiverAddress, [serialNumber1, serialNumber2], [initialIssuance1, initialIssuance2]);
        const receipt = await mintTx.wait();
        ...
        expect((receipt?.events as any)[0].args.balances).to.eql([BigNumber.from(initialIssuance1), BigNumber.from(initialIssuance2)]);
    });
    ...
    it("mintBatch invalid input lengths", async () => {
        ...
        await erc1155Precompile
            .connect(bobSigner).mintBatch(receiverAddress, [serialNumber1], [initialIssuance1, initialIssuance2])
            .catch((err: any) => {
                errorFound = true;
    ...
    it("balanceOf, balanceOfBatch", async () => {
        ...
        expect(await erc1155Precompile.balanceOfBatch([bobSigner.address, bobSigner.address], [serialNumber1, serialNumber2])).to.eql([ BigNumber.from(initialIssuance1),  BigNumber.from(initialIssuance2)]);
    });
    ...
    it("setBaseURI", async () => {
        ...
        const mintTx = await erc1155Precompile.connect(bobSigner).setBaseURI( ethers.utils.hexlify(ethers.utils.toUtf8Bytes(newMetadataPath)));
        const receipt = await mintTx.wait();
    ...
    it("setMaxSupply", async () => {
        ...
        let errorFound = false;
        await erc1155Precompile
            .connect(bobSigner).setMaxSupply(serialNumber, maxIssuance)
            .catch((err: any) => {
                errorFound = true;
    ...
    it("burnBatch", async () => {
        ...
        const burnAmount = 69;
        const tx = await erc1155Precompile.connect(bobSigner).burnBatch(bobSigner.address, [serialNumber1, serialNumber2], [burnAmount, burnAmount]);
        const receipt = await tx.wait();
    ...
    it("SafeTransferFrom approved address", async () => {
        ...
        const tx2 = await erc1155Precompile.connect(alithSigner).safeTransferFrom(bobSigner.address, alithSigner.address, serialNumber, transferAmount, callData);
        ...
        // Verify ownership
        expect(await erc1155Precompile.balanceOf(bobSigner.address, serialNumber)).to.equal(initialIssuance - transferAmount);
    ...
    it("SafeTransferFrom owner", async () => {
        ...
        const transferAmount = 69;
        const callData = ethers.utils.hexlify(ethers.utils.toUtf8Bytes("data"));
        const tx = await erc1155Precompile.safeTransferFrom(bobSigner.address, alithSigner.address, serialNumber, transferAmount, callData);
        const receipt = await tx.wait();
    ...
    it("SafeBatchTransferFrom owner", async () => {
        ...
        const callData = ethers.utils.hexlify(ethers.utils.toUtf8Bytes("data"));
        const tx = await erc1155Precompile.safeBatchTransferFrom(bobSigner.address, alithSigner.address, [serialNumber1, serialNumber2], [transferAmount1, transferAmount2], callData);
        const receipt = await tx.wait();
        ...
        expect((receipt?.events as any)[0].args.to).to.equal(alithSigner.address);
        expect((receipt?.events as any)[0].args.ids).to.eql([BigNumber.from(serialNumber1), BigNumber.from(serialNumber2)]);
        expect((receipt?.events as any)[0].args.balances).to.eql([BigNumber.from(transferAmount1), BigNumber.from(transferAmount2)]);

        // Verify ownership
        expect(await erc1155Precompile.balanceOf(bobSigner.address, serialNumber1)).to.equal(initialIssuance1 - transferAmount1);
        expect(await erc1155Precompile.balanceOf(alithSigner.address, serialNumber1)).to.equal(transferAmount1);
        expect(await erc1155Precompile.balanceOf(bobSigner.address, serialNumber2)).to.equal(initialIssuance2 - transferAmount2);
    ...
    it("BalanceOf, BalanceOfBatch via Proxy", async () => {
        ...
        // Verify balanceOfBatch works
        expect(await precompileCaller.balanceOfBatchProxy([bobSigner.address, bobSigner.address], [serialNumber1, serialNumber2])).to.eql([ BigNumber.from(initialIssuance1),  BigNumber.from(initialIssuance2)]);
    });
    ...
    it("SafeTransferFrom approved address via Proxy", async () => {
        ...
        const transferAmount = 69;
        const callData = ethers.utils.hexlify(ethers.utils.toUtf8Bytes("data"));
        const tx2 = await precompileCaller.safeTransferFromProxy(bobSigner.address, alithSigner.address, serialNumber, transferAmount, callData);
        await tx2.wait();
    ...
    it("SafeBatchTransferFrom approved address via Proxy", async () => {
        ...
        const transferAmount2 = 71;
        const callData = ethers.utils.hexlify(ethers.utils.toUtf8Bytes("data"));
        const tx2 = await precompileCaller.safeBatchTransferFromProxy(bobSigner.address, alithSigner.address, [serialNumber1, serialNumber2], [transferAmount1, transferAmount2], callData);
        await tx2.wait();
