...
describe("ERC721 Gas Estimates", function () {
    ...
    after(async () => {
        ...
        saveTxEstimates(allEstimates, "ERC721/TxCosts.md", "ERC721 Precompiles");
    ...
    it("set approval for all gas estimates", async () => {
        ...
        // precompile fee cost
        let balanceBefore = await alithSigner.getBalance();
        let tx = await erc721Precompile.connect(alithSigner).setApprovalForAll(bobSigner.address, true, {gasLimit: 782506});
        const precompileReceipt = await tx.wait();
        let balanceAfter = await alithSigner.getBalance();
        const precompileFeeCost = balanceBefore.sub(balanceAfter);
        ...
        tx = await erc721Contract
            .connect(alithSigner)
            .setApprovalForAll(bobSigner.address, true, {gasLimit: 782506});
        const contractReceipt = await tx.wait();
        balanceAfter = await alithSigner.getBalance();
        const contractFeeCost = balanceBefore.sub(balanceAfter);
        ...
        // allEstimates["setApprovalForAll"] = {
        //     Contract: {
        //         estimate: contractGasEstimate,
        //         actual: contractReceipt.gasUsed
        //     },
...
//     });
    ...
    it("renounceOwnership gas estimates", async () => {
        ...
        allEstimates["renounceOwnership"] = {
            Contract: {
                estimate: contractGasEstimate,
                actual: contractReceipt.gasUsed
            },
