...
describe("ERC721 Precompile", function () {
	...
	it("approve and setApprovalForAll via EVM", async () => {
		...
		const approve = await precompileCaller
			.connect(bobSigner)
			.approveProxy(aliceSigner.address, serial_number, { gasLimit: 50000 });
		await approve.wait();

		// Check approval through proxy
		expect(await precompileCaller.getApprovedProxy(serial_number)).to.equal(
			aliceSigner.address
		);
		...
		expect(
			await precompileCaller.isApprovedForAllProxy(
				precompileCaller.address,
				aliceSigner.address
			)
		).to.equal(false);
		...
		const approvalForAll = await precompileCaller
			.connect(bobSigner)
			.setApprovalForAllProxy(aliceSigner.address, true, { gasLimit: 50000 });
	...
	it("initialize collection", async () => {
		...
		expect((receipt?.events as any)[0].args.collectionOwner).to.equal(
			aliceSigner.address
		);
		expect((receipt?.events as any)[0].args.precompileAddress).to.equal(
			precompile_address
		);
	});
});
