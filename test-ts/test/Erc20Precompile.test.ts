...
describe("ERC20 Precompile", function () {
	...
	before(async () => {
		...
		).connect(jsonProvider); // 'development' seed
		xrpToken = new Contract(xrpTokenAddress, ERC20_ABI, seedSigner);

		let factory = new ContractFactory(
			PrecompileCaller.abi,
			PrecompileCaller.bytecode,
			seedSigner
		);
	...
	it("XRP transfer, balanceOf", async () => {
		...
		expect(await xrpToken.balanceOf(receiverAddress)).to.be.equal(
			transferAmount
		);
		// Account should be decremented by the sent amount + fees
		expect(await xrpToken.balanceOf(seedSigner.address)).to.be.lessThan(
	...
	it("XRP transfer amounts via EVM", async () => {
		...
		await endowTx.wait();
		expect(await jsonProvider.getBalance(precompileCaller.address)).to.be.equal(
			endowment
		);
		const receiverAddress = await Wallet.createRandom().getAddress();
