...
fn convert<T: pallet_nft::Config>(
	...
	// TODO Split across blocks
	token_ownership_old.iter().for_each(|ownership| {
		for serial_number in ownership.owned_serials.iter() {
			let token_locks = old::TokenLocks::<T>::get((collection_id, serial_number));
			let token_utility_flags =
				old::TokenUtilityFlags::<T>::get((collection_id, serial_number));
		...
		pallet_nft::OwnedTokens::<T>::insert(
			ownership.owner.clone(),
			collection_id,
			ownership.owned_serials.clone(),
		);
	});
...
impl<T: pallet_nft::Config + pallet_migration::Config> MigrationStep for NftMigration<T> {
	...
	fn step(last_key: Option<Vec<u8>>) -> MigrationStepResult {
		...
		if let Some((key, old)) = iter.next() {
			pallet_nft::CollectionInfo::<T>::insert(key, convert::<T>(key, old));
