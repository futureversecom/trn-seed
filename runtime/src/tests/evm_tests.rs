...
mod batch_evm_support {
	...
	#[test]
	fn proxy_extrinsic_with_batch_all_evm_and_non_evm_calls_works() {
		ExtBuilder::default().build().execute_with(|| {
			System::set_block_number(1);
	...
	#[test]
	fn batch_with_multiple_evm_calls_works() {
		ExtBuilder::default().build().execute_with(|| {
			System::set_block_number(1);
			let new_account = get_account_id_from_seed::<ecdsa::Public>("BatchTest");
	...
	#[test]
	fn batch_with_evm_and_non_evm_calls_works() {
		ExtBuilder::default().build().execute_with(|| {
			System::set_block_number(1);
			let new_account = get_account_id_from_seed::<ecdsa::Public>("MixedBatchTest");
			...
			// Create batch call containing both EVM and non-EVM calls
			let batch_call = RuntimeCall::Utility(pallet_utility::Call::batch {
				calls: vec![remark_call_1, evm_call, remark_call_2],
			});
	...
	fn proxy_extrinsic_with_batch_evm_and_non_evm_calls_works() {
		ExtBuilder::default().build().execute_with(|| {
			...
			// Test the proxy call with mixed EVM and non-EVM calls in batch
			assert_ok!(FeeProxy::call_with_fee_preferences(
				RawOrigin::Signed(new_account).into(),
				payment_asset,
				max_payment,
